...
(function() {
    ...
    style.textContent = `
        ...
        /* 左右2カラム */
        #rf-content { display: flex; flex-direction: row; align-items: flex-start; }
        #rf-content.hide { display: none; }
        #rf-left { min-width: 130px; flex-shrink: 0; padding-right: 10px; }
        #rf-vdivider { width: 1px; background: #ddd; align-self: stretch; flex-shrink: 0; }
        #rf-right { min-width: 240px; flex: 1; padding-left: 10px; }


        /* フィルター（元スタイル維持） */
        #rf-panel label { display: block; margin: 4px 0; cursor: pointer; }
        #rf-panel input[type="radio"] { margin-right: 6px; }
        ...
        .rf-btn {
            ...
            width: 100%; border: 1px solid #232f3e; background: #fff; color: #232f3e;
        }
        ...
        .rf-block-error { border-color: #f44336; color: #f44336; }


        /* ユーティリティ */
        .rf-hide { display: none !important; }
        .rf-notification {
            position: fixed; top: 50px; right: 150px; padding: 10px 15px;
    ...
    function getSavedFilter() { return localStorage.getItem(STORAGE_KEY) || 'ALL'; }
    function saveFilter(key)  { localStorage.setItem(STORAGE_KEY, key); }
    ...
    function getTable() { return document.getElementById('cspDATable'); }
    function getRows()  { const t = getTable(); return t ? t.querySelectorAll('tbody tr') : []; }
    ...
    function doFilter() {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (shouldShow) { row.classList.remove('rf-hide'); visible++; }
            else            { row.classList.add('rf-hide'); }
        }
    ...
    function cacheNamesForFilter(filterKey) {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (nameTd) { const n = nameTd.textContent.trim(); if (n) names.add(n); }
        }
    ...
    function cacheAllCycleNames() {
        FILTERS.forEach(f => { if (f.key !== 'ALL') cacheNamesForFilter(f.key); });
    }
        ...
        function processBatch() {
            ...
            for (; index < end; index++) {
                ...
                if (row.classList.contains('rf-hide')) {
                    ...
                    if (nameTd) { nameTd.style.color = ''; nameTd.style.fontWeight = ''; }
                    continue;
                }
                ...
                let newColor = '', isActive = false, isDuplicate = false;
                ...
                if (!endTime || endTime >= now) {
                    ...
                    if (isActive)         newColor = ACTIVE_COLOR;
                    else if (isDuplicate) newColor = DUPLICATE_COLOR;
                }


                row.style.backgroundColor = newColor;
                if (nameTd) {
                    if (isActive && isDuplicate) { nameTd.style.color = '#c00'; nameTd.style.fontWeight = 'bold'; }
                    else                         { nameTd.style.color = '';     nameTd.style.fontWeight = ''; }
                }
            }
            if (index < rows.length) setTimeout(processBatch, 0);
            else { updateDuplicateCount(duplicateCount, currentKey); isProcessing = false; }
        }
        processBatch();
    }
    ...
    function copyTableData() {
        ...
        table.querySelectorAll('tbody tr').forEach(row => {
            ...
            if (cells.length > 0) { data.push(cells.join('\t')); copiedCount++; }
        });
    ...
    /* ======================================================
       Block Counter — ロジック（右カラム表示）
    ====================================================== */


    function getBlockSection() { return document.getElementById('rf-block-section'); }


    function calculateBlocks() {
        const table = getTable();
        if (!table) { showBlockError('テーブルが見つかりません'); return; }


        const headers = table.querySelectorAll('th');
        let timeColumnIndex = -1, durationColumnIndex = -1;
        ...
        if (timeColumnIndex === -1) { showBlockError('「開始時刻」列が見つかりません'); return; }
        ...
        table.querySelectorAll('tbody tr').forEach(row => {
            ...
            if (timeFormatData[cellText]) {
                ...
                timeFormatData[cellText] = { count: 1, durations: duration ? [duration] : [] };
            }
        });
    ...
    function showBlockError(message) {
        ...
        // DOM操作でHTMLを組み立て（innerHTML の文字列代入を避ける）
        section.innerHTML = '';
    ...
    function showBlockResult(timeFormatData, hasDurationColumn) {
        ...
        const selectedFilter = document.querySelector('#rf-panel input:checked');
        const filterLabel    = (selectedFilter && selectedFilter.value !== 'ALL') ? selectedFilter.value : '全て';
    ...
    /* ======================================================
       UI パネル — 左右2カラム統合版
       ※ innerHTML は使わず DOM操作で構築（文字列混入バグを根絶）
    ====================================================== */
    ...
    function waitForTable() {
        const table = getTable();
        if (table && table.querySelector('tbody tr')) init();
        else setTimeout(waitForTable, 1000);
    }
